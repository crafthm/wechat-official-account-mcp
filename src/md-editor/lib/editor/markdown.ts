...
// 可选依赖 - 使用条件导入
// 注意：这些依赖需要安装，如果未安装会导致运行时错误
// 建议运行: npm install @codemirror/autocomplete @codemirror/commands @codemirror/language-data @codemirror/search

// 使用函数来延迟加载，避免在模块加载时出错
function getOptionalImports() {
  let closeBrackets: any, closeBracketsKeymap: any = [];
  let defaultKeymap: any = [], history: any, historyKeymap: any = [];
  let languages: any = [];
  let highlightSelectionMatches: any;
  ...
  return { closeBrackets, closeBracketsKeymap, defaultKeymap, history, historyKeymap, languages, highlightSelectionMatches };
}
...
export function markdownSetup(options?: MarkdownKeymapOptions) {
  const { closeBrackets, closeBracketsKeymap, defaultKeymap, history, historyKeymap, languages, highlightSelectionMatches } = getOptionalImports();
  ...
  // 缩进标记（需要 @replit/codemirror-indentation-markers）
  // indentationMarkers(),
  ...
  extensions.push(Prec.high(markdownKeymap(options)));

  // 默认快捷键（如果可用）
  const keymaps: any[] = [];
  if (defaultKeymap && defaultKeymap.length > 0) {
    keymaps.push(...defaultKeymap);
  }
  if (historyKeymap && historyKeymap.length > 0) {
    keymaps.push(...historyKeymap);
  }
  if (closeBracketsKeymap && closeBracketsKeymap.length > 0) {
